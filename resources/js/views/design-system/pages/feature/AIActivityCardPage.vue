...
<template>
  ...
  >
    ...
    <!-- ═══════════════════════════════════════════════════════════════
         ...
         ════════════════════════════════════════════════════════════════ -->
    <section class="mb-20">
      <VariantFrame label="A" caption="Stacked rail — collapsed row with chevron; expanded pushes down Trigger → Steps → Output → Actions in a clean magazine column.">
    ...
    <!-- ═══════════════════════════════════════════════════════════════
         VARIANT B — Split pane (desktop) / stacked (mobile)
         ════════════════════════════════════════════════════════════════ -->
    <section class="mb-20">
      <VariantFrame label="B" caption="Split pane — desktop shows Steps (60%) beside Output + Actions (40%) without scrolling. Mobile collapses to stacked like A.">
        <div class="w-full space-y-10">
          ...
          <div class="rounded-2xl border p-6 space-y-6" :style="{ background: L.surfaceApp, borderColor: L.borderSubtle }">
            ...
            <div class="space-y-3">
              ...
              >
                ...
                <span
                  ...
                  :style="{ background: L.status.success.soft, color: L.status.success.bold }"
                >1.2s · Done</span>
                <ChevronUpIcon v-if="expB_lt" class="w-4 h-4 flex-shrink-0" :style="{ color: L.inkTertiary }" />
                <ChevronDownIcon v-else class="w-4 h-4 flex-shrink-0" :style="{ color: L.inkTertiary }" />
              </div>

              <!-- EXPANDED — split pane on md+ -->
              <div
                v-if="expB_lt"
                class="rounded-[20px] border overflow-hidden"
                :style="{ background: L.surfaceRaised, borderColor: L.borderSubtle, boxShadow: SH_LT }"
              >
                <!-- Trigger bar -->
                <div class="px-5 pt-4 pb-3 border-b flex items-start gap-2" :style="{ borderColor: L.borderSubtle }">
                  <SparklesIcon class="w-4 h-4 flex-shrink-0 mt-0.5" :style="{ color: L.accents.lavender.bold }" />
                  <p class="text-sm italic" :style="{ color: L.inkSecondary }">"Add a task for Emma tomorrow to take out trash"</p>
                </div>
                ...
                <div class="flex flex-col md:flex-row">
                  ...
                  <!-- LEFT — Steps (60%) -->
                  <div
                    class="flex-[3] px-5 pt-4 pb-4 border-b md:border-b-0 md:border-r"
                  ...
                  <!-- RIGHT — Output + Actions (40%) -->
                  <div class="flex-[2] px-5 pt-4 pb-4 flex flex-col gap-4">
                    <div>
                      <p class="text-[11px] font-semibold uppercase tracking-widest mb-2" :style="{ color: L.inkTertiary }">Output</p>
                      <div class="rounded-xl px-4 py-3" :style="{ background: L.accents.lavender.soft }">
                        <p class="text-sm font-semibold" :style="{ color: L.inkPrimary }">Take out trash</p>
                        <p class="text-[11px] mt-0.5" :style="{ color: L.inkSecondary }">Assigned to Emma · Due tomorrow</p>
                        <span
            ...
            <p class="text-[11px]" :style="{ color: L.inkTertiary }">
              At 375px the two-column layout stacks vertically (flex-col). At md (768px+) it splits 60/40.
            </p>
          </div>
            ...
            <div class="space-y-3">
              ...
              >
                ...
                <span
                  ...
                  :style="{ background: D.status.success.soft, color: D.status.success.bold }"
                >1.2s · Done</span>
                <ChevronUpIcon v-if="expB_dk" class="w-4 h-4 flex-shrink-0" :style="{ color: D.inkTertiary }" />
                <ChevronDownIcon v-else class="w-4 h-4 flex-shrink-0" :style="{ color: D.inkTertiary }" />
              </div>
              ...
              <div
                ...
                :style="{ background: D.surfaceRaised, borderColor: D.borderSubtle, boxShadow: SH_DK }"
              >
                <div class="px-5 pt-4 pb-3 border-b flex items-start gap-2" :style="{ borderColor: D.borderSubtle }">
                  <SparklesIcon class="w-4 h-4 flex-shrink-0 mt-0.5" :style="{ color: D.accents.lavender.bold }" />
                  <p class="text-sm italic" :style="{ color: D.inkSecondary }">"Add a task for Emma tomorrow to take out trash"</p>
                </div>
                ...
                <div class="flex flex-col md:flex-row">
                  ...
                  <div class="flex-[2] px-5 pt-4 pb-4 flex flex-col gap-4">
                    <div>
                      <p class="text-[11px] font-semibold uppercase tracking-widest mb-2" :style="{ color: D.inkTertiary }">Output</p>
                      <div class="rounded-xl px-4 py-3" :style="{ background: D.accents.lavender.soft }">
                        <p class="text-sm font-semibold" :style="{ color: D.inkPrimary }">Take out trash</p>
                        <p class="text-[11px] mt-0.5" :style="{ color: D.inkSecondary }">Assigned to Emma · Due tomorrow</p>
                        <span
    ...
    <!-- ═══════════════════════════════════════════════════════════════
         VARIANT C — Timeline / trace view
         ════════════════════════════════════════════════════════════════ -->
    <section class="mb-20">
      <VariantFrame label="C" caption="Timeline trace — a 2px vertical connector line links trigger → each step → output, like a Git commit graph. Power-user audit view.">
        <div class="w-full space-y-10">
            ...
            <div class="space-y-3 max-w-[600px]">
              ...
              >
                ...
                <span
                  ...
                  :style="{ background: L.status.success.soft, color: L.status.success.bold }"
                >1.2s · Done</span>
                <ChevronUpIcon v-if="expC_lt" class="w-4 h-4 flex-shrink-0" :style="{ color: L.inkTertiary }" />
                <ChevronDownIcon v-else class="w-4 h-4 flex-shrink-0" :style="{ color: L.inkTertiary }" />
              </div>
              ...
              >
                <div class="px-5 pt-5 pb-5">
                  ...
                  <div class="relative pl-8">
                    ...
                    <div class="relative mb-5">
                      <div
                        ...
                        :style="{ background: L.accents.lavender.soft, border: `2px solid ${L.accents.lavender.bold}` }"
                      >
                        <SparklesIcon class="w-3 h-3" :style="{ color: L.accents.lavender.bold }" />
                      </div>
                      <div>
                        <p class="text-[11px] font-semibold uppercase tracking-widest mb-1" :style="{ color: L.inkTertiary }">Trigger</p>
                        <p class="text-sm italic" :style="{ color: L.inkSecondary }">"Add a task for Emma tomorrow to take out trash"</p>
                      </div>
                    </div>
                    ...
                    <div
                      ...
                      :key="step.label"
                      class="relative"
                      :class="i < STEPS.length - 1 ? 'mb-4' : 'mb-5'"
                    >
                      <div
                        class="absolute -left-8 w-6 h-6 rounded-full flex items-center justify-center"
                        :style="{ background: L.status.success.soft, border: `2px solid ${L.status.success.bold}` }"
                      >
                        <CheckCircleIcon class="w-3 h-3" :style="{ color: L.status.success.bold }" />
                      </div>
                      <div class="flex items-center gap-2">
                        <component :is="step.icon" class="w-4 h-4 flex-shrink-0" :style="{ color: L.inkTertiary }" />
                        <span class="text-sm font-mono" :style="{ color: L.inkPrimary }">{{ step.label }}</span>
                        <span class="text-[11px] ml-auto" :style="{ color: L.inkTertiary }">{{ step.dur }}</span>
                      </div>
                    </div>
                    ...
                    <div class="relative">
                      <div
                        ...
                        :style="{ background: L.accents.lavender.soft, border: `2px solid ${L.accents.lavender.bold}` }"
                      >
                        <CheckCircleIcon class="w-3 h-3" :style="{ color: L.accents.lavender.bold }" />
                      </div>
                      <p class="text-[11px] font-semibold uppercase tracking-widest mb-2" :style="{ color: L.inkTertiary }">Output</p>
                      <div class="rounded-xl px-4 py-3" :style="{ background: L.accents.lavender.soft }">
                        <div class="flex items-start justify-between gap-2">
                  ...
                  <!-- Actions row — outside timeline rail -->
                  <div class="flex items-center gap-2 mt-4 pt-4 border-t" :style="{ borderColor: L.borderSubtle }">
                    <button
                      class="flex items-center gap-1.5 text-sm font-medium px-3 py-1.5 rounded-lg border"
                      :style="{ borderColor: L.borderSubtle, color: L.inkSecondary, background: 'transparent' }"
                    >
                      <XMarkIcon class="w-4 h-4" />
                      Dismiss
              ...
              >
                <div class="px-5 py-4">
                  <div class="relative pl-8">
                    <div class="absolute left-[11px] top-0 bottom-0 w-0.5" :style="{ background: L.borderSubtle }" />
                    ...
                    <div class="relative mb-4">
                      ...
                      <p class="text-sm italic" :style="{ color: L.inkSecondary }">"Schedule family movie night for Friday"</p>
                    </div>
                    ...
                    <div class="relative mb-4">
                      ...
                      <div class="flex items-center gap-2">
                        <MagnifyingGlassIcon class="w-4 h-4 flex-shrink-0" :style="{ color: L.inkTertiary }" />
                        <span class="text-sm font-mono" :style="{ color: L.inkPrimary }">search_family_members</span>
                        <span class="text-[11px] ml-auto" :style="{ color: L.inkTertiary }">0.18s</span>
                      </div>
                    </div>
                    ...
                    <div class="relative mb-4">
                      ...
                      <div class="flex items-center gap-2">
                        <MagnifyingGlassIcon class="w-4 h-4 flex-shrink-0" :style="{ color: L.inkTertiary }" />
                        <span class="text-sm font-mono" :style="{ color: L.inkPrimary }">check_calendar_availability</span>
                        <span class="text-[11px] ml-auto" :style="{ color: L.inkTertiary }">0.42s</span>
                      </div>
                    </div>
                    ...
                    <div class="relative">
                      <div
                        class="absolute -left-8 w-6 h-6 rounded-full flex items-center justify-center"
                        :style="{ background: L.status.pending.soft, border: `2px solid ${L.status.pending.bold}` }"
                      >
                        <svg class="w-3 h-3 animate-spin" viewBox="0 0 24 24" fill="none" :style="{ color: L.status.pending.bold }">
                          <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" stroke-dasharray="31.4" stroke-dashoffset="10" stroke-linecap="round" />
                        </svg>
                      </div>
                      <div class="flex items-center gap-2">
                        <PlusCircleIcon class="w-4 h-4 flex-shrink-0" :style="{ color: L.status.pending.bold }" />
                        <span class="text-sm font-mono" :style="{ color: L.status.pending.bold }">create_calendar_event</span>
                        <span
                          class="text-[11px] px-2 py-0.5 rounded-full ml-auto"
                          :style="{ background: L.status.pending.soft, color: L.status.pending.bold }"
                        >running…</span>
                      </div>
                    </div>
              ...
              >
                <div class="px-5 py-4">
                  <div class="relative pl-8">
                    <div class="absolute left-[11px] top-0 bottom-0 w-0.5" :style="{ background: L.status.failed.soft }" />
                    ...
                    <div class="relative mb-4">
                      ...
                      <p class="text-sm italic" :style="{ color: L.inkSecondary }">"Add event 'Book club' to calendar"</p>
                    </div>

                    <div class="relative">
                      <div
                        class="absolute -left-8 w-6 h-6 rounded-full flex items-center justify-center"
                        :style="{ background: L.status.failed.soft, border: `2px solid ${L.status.failed.bold}` }"
                      >
                        <XCircleIcon class="w-3 h-3" :style="{ color: L.status.failed.bold }" />
                      </div>
                      <div>
                        <div class="flex items-center gap-2">
                          <PlusCircleIcon class="w-4 h-4 flex-shrink-0" :style="{ color: L.status.failed.bold }" />
                          <span class="text-sm font-mono" :style="{ color: L.status.failed.bold }">create_calendar_event</span>
                          <span
                            class="text-[11px] px-2 py-0.5 rounded-full ml-auto"
                            :style="{ background: L.status.failed.soft, color: L.status.failed.bold }"
                          >Failed</span>
                        </div>
                        <p class="text-[11px] mt-1.5" :style="{ color: L.status.failed.bold }">
                          Calendar connection not authorised — reconnect Google Calendar in Settings.
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <div class="space-y-3 max-w-[600px]">
              ...
              >
                ...
                <span
                  ...
                  :style="{ background: D.status.success.soft, color: D.status.success.bold }"
                >1.2s · Done</span>
                <ChevronUpIcon v-if="expC_dk" class="w-4 h-4 flex-shrink-0" :style="{ color: D.inkTertiary }" />
                <ChevronDownIcon v-else class="w-4 h-4 flex-shrink-0" :style="{ color: D.inkTertiary }" />
              </div>
              ...
              >
                <div class="px-5 pt-5 pb-5">
                  <div class="relative pl-8">
                    <div class="absolute left-[11px] top-0 bottom-0 w-0.5" :style="{ background: D.borderSubtle }" />

                    <!-- Trigger -->
                    <div class="relative mb-5">
                      <div
                        class="absolute -left-8 w-6 h-6 rounded-full flex items-center justify-center"
                        :style="{ background: D.accents.lavender.soft, border: `2px solid ${D.accents.lavender.bold}` }"
                      >
                        <SparklesIcon class="w-3 h-3" :style="{ color: D.accents.lavender.bold }" />
                      </div>
                      <div>
                        <p class="text-[11px] font-semibold uppercase tracking-widest mb-1" :style="{ color: D.inkTertiary }">Trigger</p>
                        <p class="text-sm italic" :style="{ color: D.inkSecondary }">"Add a task for Emma tomorrow to take out trash"</p>
                      </div>
                    </div>
                    ...
                    >
                      <div
                        class="absolute -left-8 w-6 h-6 rounded-full flex items-center justify-center"
                        :style="{ background: D.status.success.soft, border: `2px solid ${D.status.success.bold}` }"
                      >
                        <CheckCircleIcon class="w-3 h-3" :style="{ color: D.status.success.bold }" />
                      </div>
                      <div class="flex items-center gap-2">
                        <component :is="step.icon" class="w-4 h-4 flex-shrink-0" :style="{ color: D.inkTertiary }" />
                        <span class="text-sm font-mono" :style="{ color: D.inkPrimary }">{{ step.label }}</span>
                        <span class="text-[11px] ml-auto" :style="{ color: D.inkTertiary }">{{ step.dur }}</span>
                      </div>
                    </div>
                    ...
                    <div class="relative">
                      <div
                        ...
                        :style="{ background: D.accents.lavender.soft, border: `2px solid ${D.accents.lavender.bold}` }"
                      >
                        <CheckCircleIcon class="w-3 h-3" :style="{ color: D.accents.lavender.bold }" />
                      </div>
                      <p class="text-[11px] font-semibold uppercase tracking-widest mb-2" :style="{ color: D.inkTertiary }">Output</p>
                      <div class="rounded-xl px-4 py-3" :style="{ background: D.accents.lavender.soft }">
                        <div class="flex items-start justify-between gap-2">
                  ...
                  <div class="flex items-center gap-2 mt-4 pt-4 border-t" :style="{ borderColor: D.borderSubtle }">
                    <button
                      class="flex items-center gap-1.5 text-sm font-medium px-3 py-1.5 rounded-lg border"
                      :style="{ borderColor: D.borderSubtle, color: D.inkSecondary, background: 'transparent' }"
                    >
                ...
                <div class="px-5 py-4">
                  <div class="relative pl-8">
                    ...
                    <div class="relative mb-4">
                      ...
                      <p class="text-sm italic" :style="{ color: D.inkSecondary }">"Schedule family movie night for Friday"</p>
                    </div>
                    <div class="relative mb-4">
                      <div
                        class="absolute -left-8 w-6 h-6 rounded-full flex items-center justify-center"
                      ...
                      <div class="flex items-center gap-2">
                        <MagnifyingGlassIcon class="w-4 h-4 flex-shrink-0" :style="{ color: D.inkTertiary }" />
                        <span class="text-sm font-mono" :style="{ color: D.inkPrimary }">search_family_members</span>
                        <span class="text-[11px] ml-auto" :style="{ color: D.inkTertiary }">0.18s</span>
                      </div>
                    </div>
                    <div class="relative mb-4">
                      <div
                      ...
                      <div class="flex items-center gap-2">
                        <MagnifyingGlassIcon class="w-4 h-4 flex-shrink-0" :style="{ color: D.inkTertiary }" />
                        <span class="text-sm font-mono" :style="{ color: D.inkPrimary }">check_calendar_availability</span>
                        <span class="text-[11px] ml-auto" :style="{ color: D.inkTertiary }">0.42s</span>
                      </div>
                    </div>
                    <div class="relative">
                      <div
                        class="absolute -left-8 w-6 h-6 rounded-full flex items-center justify-center"
                        :style="{ background: D.status.pending.soft, border: `2px solid ${D.status.pending.bold}` }"
                      >
                        <svg class="w-3 h-3 animate-spin" viewBox="0 0 24 24" fill="none" :style="{ color: D.status.pending.bold }">
                          <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" stroke-dasharray="31.4" stroke-dashoffset="10" stroke-linecap="round" />
                        </svg>
                      </div>
                      <div class="flex items-center gap-2">
                        <PlusCircleIcon class="w-4 h-4 flex-shrink-0" :style="{ color: D.status.pending.bold }" />
                        <span class="text-sm font-mono" :style="{ color: D.status.pending.bold }">create_calendar_event</span>
                        <span
                          class="text-[11px] px-2 py-0.5 rounded-full ml-auto"
                          :style="{ background: D.status.pending.soft, color: D.status.pending.bold }"
                        >running…</span>
              ...
              >
                <div class="px-5 py-4">
                  <div class="relative pl-8">
                    <div class="absolute left-[11px] top-0 bottom-0 w-0.5" :style="{ background: D.status.failed.soft }" />
                    <div class="relative mb-4">
                      <div
                        class="absolute -left-8 w-6 h-6 rounded-full flex items-center justify-center"
                        :style="{ background: D.accents.lavender.soft, border: `2px solid ${D.accents.lavender.bold}` }"
                      ...
                      <p class="text-sm italic" :style="{ color: D.inkSecondary }">"Add event 'Book club' to calendar"</p>
                    </div>
                    <div class="relative">
                      <div
                        class="absolute -left-8 w-6 h-6 rounded-full flex items-center justify-center"
                        :style="{ background: D.status.failed.soft, border: `2px solid ${D.status.failed.bold}` }"
                      >
                        <XCircleIcon class="w-3 h-3" :style="{ color: D.status.failed.bold }" />
                      </div>
                      <div>
                        <div class="flex items-center gap-2">
                          <PlusCircleIcon class="w-4 h-4 flex-shrink-0" :style="{ color: D.status.failed.bold }" />
                          <span class="text-sm font-mono" :style="{ color: D.status.failed.bold }">create_calendar_event</span>
                          <span
                            class="text-[11px] px-2 py-0.5 rounded-full ml-auto"
                            :style="{ background: D.status.failed.soft, color: D.status.failed.bold }"
                          >Failed</span>
                        </div>
                        <p class="text-[11px] mt-1.5" :style="{ color: D.status.failed.bold }">
                          Calendar connection not authorised — reconnect Google Calendar in Settings.
                        </p>
                      </div>
                    </div>
