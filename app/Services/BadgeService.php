...
class BadgeService
{
    ...
    /**
     * Ensure all default badges exist for a family, creating only missing ones.
     * Idempotent — safe to call repeatedly without creating duplicates.
     * Uses firstOrCreate keyed on family_id + name to avoid duplicates.
     *
     * @return \Illuminate\Support\Collection<Badge> The created/existing badges, keyed by name.
     */
    public static function ensureDefaultBadges(string $familyId, ?string $createdBy = null): \Illuminate\Support\Collection
    {
        $defaults = [
            // --- Tasks Completed progression ---
            ['name' => 'First Steps', 'description' => 'Complete your first task', 'icon' => 'rocket', 'color' => '#059669', 'trigger_type' => BadgeTriggerType::TasksCompleted, 'trigger_threshold' => 1, 'is_hidden' => false, 'sort_order' => 0],
