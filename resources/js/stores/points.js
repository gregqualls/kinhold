...
export const usePointsStore = defineStore("points", () => {
  ...
  const giveKudos = async (userId, reason) => {
    try {
      const response = await api.post("/points/kudos", {
        user_id: userId,
        reason,
      });
      ...
      return {
        success: false,
        error: err.response?.data?.message || "Failed to give kudos",
      };
    }
  };

  const stackKudos = async (transactionId) => {
    try {
      const response = await api.post(`/points/kudos/${transactionId}/stack`);
      await Promise.all([fetchFeed(), fetchBank(), fetchLeaderboard()]);
      return { success: true, data: response.data };
    } catch (err) {
      return {
        success: false,
        error: err.response?.data?.message || "Failed to +1 kudos",
      };
    }
  };
  ...
  const deductPoints = async (userId, points, reason) => {
    try {
      const response = await api.post("/points/deduct", {
        user_id: userId,
        points,
        reason,
      });
      ...
      return {
        success: false,
        error: err.response?.data?.message || "Failed to deduct points",
      };
    }
  };
  ...
  const createReward = async (data) => {
    try {
      ...
      return {
        success: false,
        error: err.response?.data?.message || "Failed to create reward",
      };
    }
  };
  ...
  const purchaseReward = async (rewardId) => {
    try {
      ...
      return {
        success: false,
        error: err.response?.data?.message || "Failed to purchase reward",
      };
    }
  };
  ...
  const placeBid = async (rewardId, bidAmount) => {
    try {
      const response = await api.post(`/rewards/${rewardId}/bid`, {
  ...
  const fetchBids = async (rewardId) => {
    try {
      ...
      return {
        success: false,
        error: err.response?.data?.message || "Failed to fetch bids",
      };
    }
  };
  ...
  const closeAuction = async (rewardId) => {
    try {
      ...
      return {
        success: false,
        error: err.response?.data?.message || "Failed to close auction",
      };
    }
  };
  ...
  const denyPointRequest = async (requestId) => {
    try {
      ...
      return {
        success: false,
        error: err.response?.data?.message || "Failed to deny request",
      };
    }
  };
