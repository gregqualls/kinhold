<template>
  <BaseCard class="md:col-span-2 lg:col-span-3" shadow="lg">
    <div class="flex items-center justify-between mb-4">
      <h2 class="text-lg font-semibold font-heading text-ink-primary flex items-center gap-2">
        <StarIcon class="w-5 h-5 text-sand-500" />
        Coming Up
      </h2>
      <button
        v-if="isParent"
        class="flex items-center gap-1 text-sm font-medium text-accent-lavender-bold hover:text-accent-lavender-bold transition-colors"
    ...
    <div v-else-if="upcomingEvents.length > 0" class="space-y-3">
      ...
      >
        ...
        <div class="flex-1 min-w-0">
          <p class="font-semibold text-ink-primary truncate">
            {{ event.title }}
          </p>
          <div class="flex items-center gap-2 text-xs flex-wrap">
            ...
            <span v-if="event.event_time" class="text-ink-tertiary">
              {{ formatTime(event.event_time) }}
            </span>
            <span
              v-if="event.recurrence_label"
              class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-accent-lavender-soft/40 text-accent-lavender-bold text-[10px] font-medium"
            >
        ...
        <div v-if="isParent" class="flex-shrink-0 relative">
          <button
            class="p-1 rounded-lg hover:bg-surface-overlay transition-colors text-ink-tertiary"
          ...
          <div
            ...
            class="absolute right-0 top-full mt-1 w-32 bg-surface-raised rounded-lg shadow-lg border border-border-subtle z-10 py-1"
          >
            <button
            ...
            <button
              class="w-full text-left px-3 py-1.5 text-sm text-ink-primary hover:bg-surface-sunken flex items-center gap-1.5"
            ...
            <button
              class="w-full text-left px-3 py-1.5 text-sm text-status-failed hover:bg-red-50 dark:hover:bg-red-900/20"
...
const countdownClasses = (dateStr) => {
  ...
  if (countdown === 'tomorrow') {
    return 'bg-accent-lavender-soft/40 text-accent-lavender-bold'
  }
